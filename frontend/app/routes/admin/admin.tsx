...
} from "~/validations/admin/ManageAdmin/createAdmin";
...
function Admin() {
  ...
  const createAdmin: SubmitHandler<CreateAdminValues> = async (data) => {
    console.log("yess");
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      enableSorting: false,
      cell: (props: any) => (
        <ActionCell onEdit={setUpdateAdminModal} value={props.getValue()} />
      ),
    },
  ];
  ...
  return (
    <div>
      <Table
        actionButton={
          <button
            className="bg-[#465fff] text-white w-40 rounded-xl cursor-pointer"
